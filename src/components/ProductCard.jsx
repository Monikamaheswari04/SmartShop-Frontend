...
export default function ProductCard({ product }) {
  ...
  const handleWishlist = () => {
    ...
    if (!isWishlisted) {
      ...
      el.className = "fixed top-20 right-10 bg-pink-500 text-white px-3 py-1 rounded shadow-lg z-50 animate-bounce";
  ...
  return (
    <Motion.article
      ...
      whileTap={{ scale: 0.97 }}
      className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 flex flex-col transition-colors duration-300"
    >
      <div className="relative">
        <Link to={`/product/${product.id}`} className="overflow-hidden rounded">
          <img
            src={product.image}
            alt={product.name}
            className="h-44 w-full object-cover rounded transition-transform duration-300 hover:scale-105"
          />
        </Link>
        <button
      ...
      <div className="mt-3">
        <button
          ...
          className="w-full px-3 py-2 rounded bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white transition-colors duration-300"
        >
          Add to cart
        </button>
      </div>
